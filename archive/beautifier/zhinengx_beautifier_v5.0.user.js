...
(function() {
    ...
    function generateBaseCSS() {
        return `
            ...
            :root {
                ...
                --znx-f-bg: rgba(255, 255, 255, var(--znx-F_opacity));
    ...
    function processDynamicJssClasses() {
        ...
        // 查找所有包含 jss 或 CSS Modules 哈希类 (以 _ 开头，如 _3WnwfR...) 的容器
        const candidateDivs = root.querySelectorAll('div[class*="jss"]:not(.MuiPaper-root), div[class^="_"]:not(.MuiPaper-root), div[class*=" _"]:not(.MuiPaper-root)');
        ...
        // 【关键修复】做题时，自动隐藏底下被覆盖的蓝色主导航条，杜绝重影透出
        const isDoingQuestions = !!document.querySelector('div[class*="_3WnwfR"]');
        const mainNav = document.getElementById('navigation-pills') || document.getElementById('moduleHeader') || document.querySelector('#root .MuiTabs-root');
        if (mainNav) {
            mainNav.style.opacity = isDoingQuestions ? '0' : '1';
            mainNav.style.pointerEvents = isDoingQuestions ? 'none' : 'auto';
            mainNav.style.transition = 'opacity 0.2s';
        }
