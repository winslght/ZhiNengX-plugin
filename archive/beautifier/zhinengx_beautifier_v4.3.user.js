...
(function() {
    ...
    function generateBaseCSS() {
        return `
            ...
            /* 全局透明化，铲除原生底色，但严格排除看板娘插件 */
            body, #root, #root > div:not(#waifu):not(#waifu-toggle):not(#waifu-tool):not(.waifu), #root > div > div {
    ...
    function processDynamicJssClasses() {
        ...
        // 动态迁移“收纳按钮”到蓝条位置
        const tabsContainer = document.querySelector('#navigation-pills .MuiTabs-flexContainer') || document.querySelector('#moduleHeader .MuiTabs-flexContainer');
        const miniBtn = document.getElementById('znx-tuner-mini');
        if (miniBtn) {
            if (tabsContainer && !tabsContainer.contains(miniBtn)) {
                tabsContainer.appendChild(miniBtn);
                miniBtn.style.position = 'static';
                miniBtn.style.margin = 'auto 0 auto 20px'; // 居中并左侧留白
            } else if (!tabsContainer && miniBtn.parentElement !== document.body) {
                // 如果蓝条消失了，退回到固定在右上角
                document.body.appendChild(miniBtn);
                miniBtn.style.position = 'fixed';
                miniBtn.style.top = '80px';
    ...
    function injectTunerPanel() {
        ...
        miniBtn.id = 'znx-tuner-mini';
        miniBtn.className = 'MuiButtonBase-root MuiTab-root MuiTab-textColorInherit';
        miniBtn.innerHTML = `
            <span class="MuiTab-wrapper" style="flex-direction: row; display: flex; align-items: center; justify-content: center; gap: 4px;">
                <svg class="MuiSvgIcon-root" focusable="false" viewBox="0 0 24 24" aria-hidden="true" style="width: 20px; height: 20px; fill: currentColor;">
                    <path d="M3 17v2h6v-2H3zM3 5v2h10V5H3zm10 16v-2h8v-2h-8v-2h-2v6h2zM7 9v2H3v2h4v2h2V9H7zm14 4v-2H11v2h10zm-6-4h2V7h4V5h-4V3h-2v6z"></path>
                </svg>
                <span style="font-size: 14px; font-weight: 500;">美化</span>
            </span>
        `;
        Object.assign(miniBtn.style, {
            cursor: 'pointer', zIndex: '9999999',
            opacity: '0.7', transition: 'opacity 0.2s',
            minWidth: 'auto', padding: '6px 12px',
            display: 'block', // 默认显示收纳按钮
            position: 'fixed', top: '80px', right: '20px', left: 'auto' // 默认位置
        });
        miniBtn.onmouseover = () => miniBtn.style.opacity = '1';
        miniBtn.onmouseout = () => miniBtn.style.opacity = '0.7';
        document.body.appendChild(miniBtn);
