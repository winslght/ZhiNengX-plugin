...
(function() {
    ...
    const defaultParams = {
        ...
        Dark_glass:  { val: 1,    min: 0, max: 1,   step: 1,    label: '🌙 深色毛玻璃模式 (1开 0关)' },
        Dark_dim:    { val: 0.65, min: 0, max: 0.98,step: 0.05, label: '🌙 壁纸暗化遮光度' },
    };
    ...
    function updateCSSVariables() {
        ...
        root.classList.toggle('znx-dark-mode', params.Dark_glass.val === 1);
        
        // 动态直接操纵遮光的 opacity 属性，打破 Dark Reader 插件的 CSS 拦截！
        const overlay = document.getElementById('znx-anime-overlay');
        if (overlay) {
            overlay.style.opacity = params.Dark_dim.val;
        }
    }
    ...
    function generateBaseCSS() {
        return `
            ...
            /* 自动感应 Dark Reader 插件或开启深色模式，切换为黑曜石深色毛玻璃 */
