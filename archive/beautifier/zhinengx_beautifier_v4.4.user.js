...
(function() {
    ...
    function processDynamicJssClasses() {
        ...
        if (miniBtn) {
            if (tabsContainer && !tabsContainer.contains(miniBtn)) {
                ...
                // 智能偷取原生类名，实现 100% 原生融合
                const sibling = tabsContainer.querySelector('.MuiTab-root');
                if (sibling) {
                    let newClass = sibling.className;
                    newClass = newClass.replace(/Mui-selected/g, ''); // 剔除高亮状态
                    miniBtn.className = newClass;
                    const siblingWrapper = sibling.querySelector('.MuiTab-wrapper');
                    const myWrapper = miniBtn.querySelector('.MuiTab-wrapper');
                    if (siblingWrapper && myWrapper) {
                        myWrapper.className = siblingWrapper.className;
                    }
                }
                // 清除占位样式，完全交给页面原生的 JSS 接管排版
                miniBtn.style.position = '';
                miniBtn.style.top = '';
                miniBtn.style.right = '';
                miniBtn.style.left = '';
                miniBtn.style.margin = '';
                miniBtn.style.padding = '';
                miniBtn.style.minWidth = '';
    ...
    function injectTunerPanel() {
        ...
        miniBtn.onmouseout = () => miniBtn.style.opacity = '0.8';
        document.body.appendChild(miniBtn);
